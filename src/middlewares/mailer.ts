...
import { Tickets, User, Movie } from "../models";
...
//       Seats: ${ticket.seats}
//       Bank: ${ticket.bank}
//       Total Price: ${ticket.totalPrice}VND
...
export const sendingBill = async (userEmail: string, ticketCode: string) => {
  try {
    ...
    if (ticket && ticket["planScreenMovie.roomCode"]) {
      ...
      const user = await User.findOne({ where: { userCode: ticket.userCode }, attributes: ["name"], raw: true });
      const movie = await Movie.findOne({ where: { movieCode: ticket["planScreenMovie.movieCode"] }, attributes: ["title"], raw: true });

      if (!user || !movie) {
        return {
          errCode: 2,
          message: "User or movie information not found",
        };
      }

      // Tạo nội dung bill với tên người dùng và tên phim
      const billContent = `
      Thank you for purchasing tickets from us!
